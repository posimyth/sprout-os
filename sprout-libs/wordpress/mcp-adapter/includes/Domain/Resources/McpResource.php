...
class McpResource {
	...
	public function get_mcp_server(): McpServer {
		if ( null === $this->mcp_server ) {
			throw new \RuntimeException( 'MCP server has not been set on this resource instance.' ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}
	...
	public function validate( string $context = '' ) {
		if ( null === $this->mcp_server ) {
			return new \WP_Error(
				...
				esc_html__( 'MCP server must be set before validating a resource.', 'sprout-os' )
			);
		}
