...
class McpPrompt {
		...
		if ( ! $ability ) {
			return new \WP_Error(
				...
				sprintf(
					...
					esc_html__( "WordPress ability '%s' does not exist.", 'sprout-os' ),
	...
	public function validate( string $context = '' ) {
		if ( null === $this->mcp_server ) {
			return new \WP_Error(
				...
				esc_html__( 'MCP server must be set before validating a prompt.', 'sprout-os' )
			);
		}
	...
	public function get_mcp_server(): McpServer {
		if ( null === $this->mcp_server ) {
			throw new \RuntimeException( 'MCP server has not been set on this prompt instance.' ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}
	...
	public function execute_direct( array $arguments ): array {
		throw new \Exception( 'This prompt does not support direct execution' ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
	}
	...
	public function check_permission_direct( array $arguments ): bool {
		throw new \Exception( 'This prompt does not support direct permission checking' ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
	}
}
